...
\textbf{Actor:} Mobile AI\smallskip\\
AI acquires player as a target. System checks if any other mobile AI is targeting that player, if not changes the background music for the targeted player to a faster tempo track. If the player is already targeted no change to the background music for that player is made. If the AI loses track of the target and is the only mobile AI targeting that player, the player's background music is set back to the default.
